...
export const generatePages = async (config: Required<IPostsConfig>) => {
  const { outDir, lang, pageSize, homepage, postCount } = config;
...
<PageView :pagination="${i}" :total="${pageCount}" :size="${pageSize}" :homepage="${homepage}" />
`.trim();
...
layout: page
---
<HomeView imgUrl="/hero.svg" title="XKgg Docs" desc="记录技术与生活" />
    `.trim();
